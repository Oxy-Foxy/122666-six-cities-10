...
function RatingInput({title, value, checked, fieldChangeHandle}: RatingInputProps):JSX.Element {
  return (
    <>
      <input className="form__rating-input visually-hidden" name="rating" value={value} id={`${value}-stars`} type="radio" checked={checked} onChange={fieldChangeHandle} />
  ...
  return (
    <form className="reviews__form form" action="#" method="post" onSubmit={handleSubmit}>
      ...
      <div className="reviews__rating-form form__rating">
        {radioInputs.map((item) => <RatingInput key={`${item.title}-${item.value}`} title={item.title} value={item.value} checked = {formData.rating === item.value} fieldChangeHandle={fieldChangeHandle}/>)}
      </div>
      <textarea className="reviews__textarea form__textarea" id="review" name="review" value={formData.review} placeholder="Tell how was your stay, what you like and what can be improved" onChange={fieldChangeHandle}></textarea>
